...
# =========================================================
# I. 설정 및 환경 변수 (CONFIG.INI 파일에 의존)
# =========================================================
...
# --- config.ini 파일 로딩 함수 (configparser 사용) ---
def load_config():
    ...
    try:
        ...
        # 토마토 오류가 아닌 다른 configparser 오류 처리
        st.error(f"❌ 오류: config.ini 파일 내용이 잘못되었습니다. [SETTINGS] 섹션이나 코드를 확인하세요. ({e})")
        st.stop()
...
def main():
    ...
    # 1-3. 상세 주소 입력 (동, 번지)
    # ⚠️ 사용자가 입력하는 최종 주소의 유효성 검사가 중요합니다.
    ...
    if st.button("🚨 위험도 진단 시작", use_container_width=True, type="primary"):
        ...
        # ------------------ 입력 포맷 검증 로직 ------------------
        # 입력된 상세 주소가 '단어 단어' 형태로 최소 2개 이상의 구성요소인지 검증 (동 + 번지)
