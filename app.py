...
import configparser  # 로컬 개발 시 config.ini 읽을 때 사용
import sys
import urllib.parse
import re
...
# 배포 시에는 자신의 스트림릿 앱 도메인으로 수정해도 좋습니다.
# 예: "https://my-realestate-app.streamlit.app"
DOMAIN = "http://localhost"


# =========================================================
# I-1. 설정 로딩 함수 (Secrets 우선, 없으면 config.ini)
# =========================================================
...
def load_config():
    ...
    try:
        # secrets에 키가 있으면 배포 환경으로 판단
        if hasattr(st, "secrets") and "VWORLD_KEY" in st.secrets:
            v_key = st.secrets["VWORLD_KEY"].strip()
            allowed_codes_str = st.secrets.get("ALLOWED_CODES", "")
            market_ratio_str = str(st.secrets.get("MARKET_RATIO", "1.4"))

            allowed_codes = [c.strip() for c in allowed_codes_str.split(",") if c.strip()]
            ratio = float(market_ratio_str)

            if not v_key or not allowed_codes:
                raise ValueError("secrets에 인증키 또는 담당 지역 코드가 비어 있습니다.")

            return v_key, ratio, allowed_codes
    except Exception as e:
        st.error(f"❌ 오류: Streamlit secrets 로딩 실패: {e}")
        st.stop()
    ...
    try:
        ...
        if not config.has_section("SETTINGS"):
            raise ValueError("config.ini에 [SETTINGS] 섹션이 없습니다.")
        ...
        if not v_key or not allowed_codes:
            raise ValueError("config.ini에 인증키 또는 담당 지역 코드가 비어 있습니다.")

        return v_key, ratio, allowed_codes

    except Exception as e:
        st.error(f"❌ 오류: 설정 파일(config.ini / secrets) 로딩 실패: {e}")
        st.stop()
...
def main():
    ...
    if not filtered_districts:
        st.error("❌ 오류: 설정된 담당 지역 코드에 해당하는 시/군/구가 없습니다. ALLOWED_CODES 값을 확인하세요.")
        return
    ...
    if st.button("🚨 위험도 진단 시작", use_container_width=True, type="primary"):
        ...
        with st.spinner("데이터를 조회하고 위험도를 판정 중입니다..."):
            ...
            if not pnu_code:
                st.error("❌ 오류: V-World에서 PNU 코드 변환에 실패했습니다. 입력 주소 조합이나 API 키/도메인 설정을 확인해 주세요.")
                return
