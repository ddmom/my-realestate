...
# =========================================================
# I. 설정 및 환경 변수 (st.secrets 로드)
# =========================================================
# API Endpoint URLs
APART_PRICE_URL = "https://api.vworld.kr/ned/data/getApartHousingPriceAttr"
SEARCH_URL = "http://api.vworld.kr/req/search"
DOMAIN = "http://localhost" # V-World API 호출 시 사용 (Key 등록 시 필요)

# 전국 행정구역 데이터 (원본 DB 역할 - config.ini에서 필터링됨)
FULL_ADMIN_DB = {
    "서울특별시": {
...
def get_pnu_code(address, key, domain):
    ...
    try:
        params = {
            ...
            "format": "json", "key": key, 
            # domain 파라미터는 제거 (Streamlit 클라우드 배포 오류 방지)
        }
...
def get_latest_official_price(pnu, key, target_year="2024", url=APART_PRICE_URL, domain=DOMAIN):
    ...
    # Referer 헤더 제거 (Streamlit 클라우드 배포 오류 방지)
    try:
...
def main():
    # --- 1. 설정 파일 로드 (st.secrets에서 키와 설정값 로드) ---
    try:
        ...
        st.error(f"❌ 설정 오류: 'st.secrets'에서 키({e})를 찾을 수 없습니다. Secrets에 config.ini 내용을 정확히 입력했는지 확인하세요.")
        st.stop()
        return
    ...
    if not filtered_districts:
        st.error("❌ 오류: 'Secrets'에 유효한 지역 코드가 없습니다. 관리자에게 문의하세요.")
        return
